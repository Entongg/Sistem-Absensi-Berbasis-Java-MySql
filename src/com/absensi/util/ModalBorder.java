...
public class ModalBorder extends Modal implements ModalBorderAction {
    ...
    public ModalBorder(Component component, String title) {
        this(component, title, new ModalBorderOption());
    }

    public ModalBorder(Component component, String title, ModalBorderOption option) {
        this(component, title, option, DEFAULT_OPTION, null);
    }

    public ModalBorder(Component component, String title, int optionType, ModalCallback callback) {
        this(component, title, new ModalBorderOption(), optionType, callback);
    }

    public ModalBorder(Component component, String title, Option[] optionsType, ModalCallback callback) {
        this(component, title, new ModalBorderOption(), -1, optionsType, callback);
    }

    public ModalBorder(Component component, String title, ModalBorderOption option, int optionType, ModalCallback callback) {
        this(component, title, option, optionType, null, callback);
    }

    public ModalBorder(Component component, String title, ModalBorderOption option, Option[] optionsType, ModalCallback callback) {
        this(component, title, option, -1, optionsType, callback);
    }

    private ModalBorder(Component component, String title, ModalBorderOption option, int optionType, Option[] optionsType, ModalCallback callback) {
    ...
    protected Option[] createOptions(int optionType) {
        ...
        if (optionType == YES_NO_OPTION) {
            options = new Option[]{new Option("No", NO_OPTION), new Option("Yes", YES_OPTION)};
    ...
    private void checkOptionType(int type) {
        if (type != DEFAULT_OPTION && type != YES_NO_OPTION && type != YES_NO_CANCEL_OPTION && type != OK_CANCEL_OPTION) {
            throw new RuntimeException("ModalBorder: option type must be one of" +
                    " ModalBorder.DEFAULT_OPTION," +
                    " ModalBorder.YES_NO_OPTION," +
                    " ModalBorder.YES_NO_CANCEL_OPTION" +
                    " or ModalBorder.OK_CANCEL_OPTION");
        }
    }
